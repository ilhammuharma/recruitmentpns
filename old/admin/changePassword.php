<?php
	...
	else
	{
		if(isset($_POST['change']))
		{
			...
			$queryCek = "SELECT * FROM loginUser WHERE username = '$username' and password = '$md5Password'";
			$resultCek = mysql_query($queryCek);

			if(mysql_num_rows($resultCek) == 0)
			{
				$_SESSION['errorPasswordMessage'] = "Password lama yang Anda masukkan cocok. Silakan ulangi lagi.";
			}
			else
			{
				$queryUpdatePassword = "UPDATE loginUser SET password = '$newMd5Password' WHERE username = '$username'";
				if($npassword != $rnpassword)
				{
					$_SESSION['errorPasswordBedaMessage'] = "Kedua password yang Anda berikan tidak sama. Silakan ulangi lagi.";
				}
				else
				{
					if(strlen($npassword) < 8){
						$_SESSION['errorLength'] = "Password kurang dari 8 huruf.";
					}
					else
					{
						$ganti = mysql_query($queryUpdatePassword);
						$_SESSION['suksesGanti'] = "Password telah berhasil diganti.";
					}
				}
			}
			...
			if(isset($_SESSION['errorPasswordMessage']) || isset($_SESSION['errorPasswordBedaMessage']) || isset($_SESSION['errorLength']))
			{
				if(isset($_SESSION['errorPasswordMessage']))
				{
					...
					echo "<a href='changePassword.php'><button class='btn btn-danger'>Kembali</button></a>";
				}
			...
			if(isset($_SESSION['suksesGanti']))
			{
				...
				echo "<a href='changePassword.php'><button class='btn btn-success'>Kembali</button></a>";
			}
				...
				?>
					...
					<form class="form-horizontal" role="form" method="post" action="changePassword.php">
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input type="password" name="cpassword" class="form-control" id="cpassword" placeholder="Password saat ini" required onchange="cekCPassword();return false;">
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input name="npassword" type="password" class="form-control" id="npassword" placeholder="Password baru" required onchange="cekNPassword();return false;">
								<div id="cekNewPasword"></div>
								<p class="help-block">Enter the new password that you want. At least of 8 letters.</p>
							</div>
						</div>
						<div class="form-group">
							<label for="rnpassword" class="col-sm-2 control-label">Retype New Password</label>
							<div class="col-sm-6">
								<input name="rnpassword" type="password" class="form-control" id="rnpassword" placeholder="Ulangi password baru" required onkeyup="cekRNPassword();return false;">
					...
					<script type="text/javascript">
						...
						function cekNPassword()
						{
							...
							if(npass.length < 8)
							{
								cekNPass.innerHTML = "<p style='color:red'>Password minimal 8 huruf.</p>";
							}
							else
							{
								cekNPass.innerHTML = "";
						...
						function cekRNPassword()
						{
							...
							if(npass != rnpass)
							{
								cek.innerHTML = "<p style='color:red'>Kedua password tidak sama.</p>";
							}
							else
							{
								cek.innerHTML = "";
