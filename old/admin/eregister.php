						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									$queryRegisterUser = "INSERT INTO loginUser values('','$email', '$username', '$md5password', '$generateKey', '$valid', 1, '$level', '')";
									$execRegister = mysql_query($queryRegisterUser);
									//echo $queryRegisterUser;
									//$queryRegisterUser = "INSERT INTO loginUser values(NULL,'$email', '$username', '$md5password', '$generateKey', '$valid', 1, '$level')";
									//$execRegister = mysql_query($queryRegisterUser);
									if($level==2)
									{
										$queryIdLogin = "SELECT * FROM loginUser WHERE email = '$email'";
										$execQueryIdLogin = mysql_query($queryIdLogin);
										$resultIdLogin = mysql_fetch_array($execQueryIdLogin);
										$idLogin = $resultIdLogin['idLogin'];
										$insertUser = "INSERT INTO userWorker(idWorker, idLogin, namaUser, emailUser, statusRekrut) VALUES(null, '$idLogin','$nama','$email', 'Screening')";
										$execInsert = mysql_query($insertUser);
										//$insertUser = "INSERT INTO userWorker(idWorker, idLogin, namaUser, emailUser, statusRekrut) VALUES(null, null,'$nama','$email', 'Screening')";
										//$execInsert = mysql_query($insertUser);
									}
									/*$queryRegisterUser = "INSERT INTO loginWorker values(NULL,'$email', '$username', '$md5password', '$generateKey', '$valid', 0)";
									$execRegister = mysql_query($queryRegisterUser);
									$queryIdLogin = "SELECT * FROM loginWorker WHERE email = '$email'";
									$execQueryIdLogin = mysql_query($queryIdLogin);
									$resultIdLogin = mysql_fetch_array($execQueryIdLogin);
									$idLogin = $resultIdLogin['idLogin'];
									$insertUser = "INSERT INTO userWorker(idWorker, idLogin, namaUser, emailUser, statusRekrut) VALUES(null, '$idLogin','$nama','$email', 'Screening')";
									...
									*/
									...
									//echo $result;
									$_SESSION['suksesMessage'] = "The registration process was successful. Please check your email inbox and follow the process given";
								}
							}
						}
				}
							...
							<div class="box-no-border">
								<?php
									if(isset($_SESSION['emailValidMessage']) || isset($_SESSION['cekEmailMessage']) || isset($_SESSION['cekUsernameMessage']) || isset($_SESSION['cekUserPasswordMessage']) || isset($_SESSION['error_captcha']))
									{
										...
										echo "<a href='eregister.php'><button class='btn btn-default'>Sign Up</button></a>";
									}
									else
									{
			...
			else
			{
				...
				<div class="container">
					<div class="row">
						<div class="col-md-9">
							<div class="box-no-border">
								<h3><strong>Account Registration for Employee</strong></h3>
									<form class="form-horizontal" role="form" method="post" action="eregister.php">
