<?php
	...
	if(isset($_POST['email']))
	{
		...
		$querySearchEmail = "SELECT * from loginWorker where email like '$email'";
		...
		if($result)
		{
			if(mysql_num_rows($result) > 0)
			{
				...
				$queryUpdate = "UPDATE loginWorker SET resetCode='$generateKey', validUntil = '$valid' WHERE email like '$email'";
				...
				$from = $emailAdmin;
				$name = $nameAdmin;
				$linkReset = $webRoot."worker/reset.php?secret=".$generateKey;
