										...
										<div class="box-no-border">
											...
											<div class="row">
												...
												<div class="row" class="form-group">
													<div class="col-md-3">
														<div class="table-responsive">
															<h4><strong><label for="permission" class="help-block">PIC FPTK</label></strong></h4>
														</div>
													</div>
													<?php
														if($_GET['approve']==4)
														{
															?>
																<form action="selectingPicProcess.php" method="POST">
														...
														else if($_GET['approve']==7)
														{
															?>
																<form action="selectingPicProcess.php" method="POST">
																	<input type="hidden" name="idFptk" value="<?php echo $_GET['idFptk']?>">
																	<input type="hidden" name="approve" value="<?php echo $_GET['approve']?>">
																	<select class="col-md-3" for= "selectPic" name="selectPic" id="selectPic"  class="form-control">
																		<?php 
																			$querySelectPic = "SELECT username FROM loginUser where idlogin=20 or idLogin=21 or idLogin=22 ";
																			//$querySelectPic .= "JOIN userWorker uw ON uw.idLogin = f.username ";
																			//$querySelectPic .= "WHERE lu.level=3";
																			$execSelectPic = mysql_query($querySelectPic);
																			while($selectPic = mysql_fetch_array($execSelectPic))
																			{
																				?><option value="<?php echo $selectPic['username'];?>"><?php echo $selectPic['username'];?></option><?php 
																			}
																		?>
																	</select>
																	<div class="row">
														...
														{
															?>
															<a href="selectingPicProcess.php?idFptk=<?php echo $_GET['idFptk']?>&approve=<?php echo $_GET['approve']?>"><button type="submit" onclick="confirmApproval()" class="btn btn-primary btn-sm" name="approval-fptk">Approve</button></a>
															<?php
																if($_GET['approve']==1 or $_GET['approve']==5 or $_GET['approve']==6)
																{
																	?><a href="selectingPicProcess.php?reject=<?php echo $_GET['idFptk']?>&r=<?php echo $_GET['approve']?>"><button type="submit" onclick="confirmReject()" class="btn btn-danger btn-sm" name="approval-fptk">Reject</button></a><?php 
																} 
																else
																{
																}
