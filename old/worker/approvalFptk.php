...
<?php
	...
	else
	{
		...
		else if(isset($_SESSION['worker']))
		{
			...
			?>
				<h3><strong>Persetujuan FPTK</strong></h3>
				<?php
					if($_GET['id']==1)
					{
									...
									<table class="table table-hover table-condensed">
										...
										<tbody>
											<?php
												...
												{
												$get_posisi=mysql_fetch_array(mysql_query("select namaKategori from kategorikerja where idKategori='".$result['posisiPemohon']."'"));
												$get_dept=mysql_fetch_array(mysql_query("select namaDepartemen from kategoridepartemen where idDepartemen='".$result['departemenPemohon']."'"));
													?>
														<tr>
															<td><a href="approvingFptk.php?idFptk=<?php echo $result['idFptk'];?>&approve=<?php echo $_GET['id']?>"><button class="btn btn-primary btn-danger">Approval</button></a></td> 
															<td><?php echo $result['nomorFptk']?></td>
															<td><?php echo $result['namaPemohon']?></td>
															<td><?php echo $result['nikPemohon']?></td>
															<td><?php echo $get_posisi['namaKategori']?></td>
															<td><?php echo $get_dept['namaDepartemen']?></td>
														</tr>
													<?php
												}
											?>
