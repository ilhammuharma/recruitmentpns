<?php
	...
	if(isset($_POST['save']))
	{
		//$pemohon = mysql_fetch_array(mysql_query("select * from loginUser where idLogin='".$_SESSION['id']."'"));
		//$nik = $_POST['nik'];
	...
	else if(isset($_POST['approval']))
	{
		...
		$keteranganPosisi = $_POST['keteranganPosisi'];
		$pic = $_POST['pic'];
		...
		else if($approve==4)
		{
			...
			{
				$updateFptk = "UPDATE fptk SET namaOdManager='$username', tanggalOdManager='$tanggal', mppPosisi='$mppPosisi', keteranganPosisi='$keteranganPosisi' WHERE idFptk='$idFptk'";
				//$updateFptk = "UPDATE fptk SET namaOdManager='$username', tanggalOdManager='$tanggal', mppPosisi='$mppPosisi', namaDirektur='Auto Approved', tanggalDirektur='$tanggal' WHERE idFptk='$idFptk'";
			}
		}
		else if($approve==5)
		...
		{
			$updateFptk = "UPDATE fptk SET namaRecruitmentSuperintendent='$username', tanggalRecruitmentSuperintendent='$tanggal', pic='$pic' WHERE idFptk='$idFptk'";
		}
		else
		{
	...
	else if(isset($_POST['reject']))
	{
		...
		$username = $_SESSION['nama'];
		$tanggal = date('Y-m-d');
		$rejectfptk = "UPDATE fptk SET rejectFptk='$username', tanggalReject='$tanggal', keteranganReject='$keteranganReject' WHERE idFptk='$id'";
		$query = mysql_query($rejectfptk);
		//echo $rejectfptk;
		if($query)
		{ 
			echo"
		...
		else
		{
			echo"
			<script>
			alert('Data could not be rejected'); 
			self.history.back();
			</script>
			";	
		}
